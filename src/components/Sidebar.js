...
function Sidebar() {
    ...
    useEffect(() => {
        if (user) {
            setCurrentRoom("IIT P");
            getRooms();
            socket.emit("join-room", "IIT P");
    ...
    function getRooms() {
        fetch("https://chat-backend-c5mb.onrender.com/rooms")
            .then((res) => res.json())
    ...
    return (
        <>
            ...
            <h2 classname="my_list">Members</h2>
            {members.map((member) => (
